<div align="center">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=500&size=30&pause=1000&color=9CAFB7&center=true&vCenter=true&width=750&lines=ls+-l+~/repositories" alt="Typing SVG" />
</div>

<p align="center">
  <a href="#">
      <img src="https://api.visitorbadge.io/api/VisitorHit?user=Michael-Steenkamp&repo=Portfolio&countColor=9CAFB7" />
   </a>
</p>

---

<div align="left">
  <details>
  <summary><code><b>drwxr-xr-x ./repo-dotfiles</b></code></summary>
  
  <table>
    <tr>
      <td><img src="https://github.com/user-attachments/assets/f2ab18cd-b7f8-419e-9efc-ea3fe2c26392" width="500" /></td>
      <td><img src="https://github.com/user-attachments/assets/f2190f7f-a885-41ae-93ae-ab10bb28d218" width="500" /></td>
      <td><img src="https://github.com/user-attachments/assets/33a99c0c-4cef-40f3-ae41-be7e4fe0c854" width="500" /></td>
      <td><img src="https://github.com/user-attachments/assets/70214f7f-e03f-41ed-973b-90df79b2841d" width="500" /></td>
    </tr>
    <tr>
      <td><img src="https://github.com/user-attachments/assets/3972a322-3232-47dc-9bc6-b2687bd0f74b" width="500" /></td>
      <td><img src="https://github.com/user-attachments/assets/a1ba2c42-ac8a-405b-a928-56ddabf2dd0b" width="500" /></td>
      <td><img src="https://github.com/user-attachments/assets/8b7a824e-4816-43ca-81cb-51d8f67f096b" width="500" /></td>
      <td><img src="https://github.com/user-attachments/assets/69dcd4bf-1789-40cf-bdf0-92e1b7014157" width="500" /></td>
    </tr>
  </table>
    
  <br>
  
  > Personal Arch Linux environment configuration. Features a custom Hyprland window manager setup, an Eww side panel, Fish shell aliases, and a fully personalized Neovim workflow configured via Lua.
  
  <div align="center">
    <img src="https://img.shields.io/badge/Arch_Linux-1793D1?style=for-the-badge&logo=arch-linux&logoColor=white" alt="Arch" />
    <img src="https://img.shields.io/badge/Hyprland-000000?style=for-the-badge&logo=linux&logoColor=white" alt="Hyprland" />
    <img src="https://img.shields.io/badge/Fish_Shell-4CAF50?style=for-the-badge&logo=gnu-bash&logoColor=white" alt="Fish" />
    <img src="https://img.shields.io/badge/Neovim-57A143?style=for-the-badge&logo=neovim&logoColor=white" alt="Neovim" />
    <img src="https://img.shields.io/badge/Lua-2C2D72?style=for-the-badge&logo=lua&logoColor=white" alt="Lua" />
  </div>
  
  <br>
  
  **[`> ./go_to_source_code`](https://github.com/Michael-Steenkamp/Dotfiles)**
  
  <hr>
  </details>

  <details>
  <summary><code><b>drwxr-xr-x ./repo-chinese</b></code></summary>
  
  <table>
    <tr>
      <td><img src="https://github.com/user-attachments/assets/4a11e13c-83b6-4e9d-91d4-abe4360b4755" width="500" /></td>
      <td><img src="https://github.com/user-attachments/assets/1b36b13a-68e0-4afe-8643-602fb22b8b7b" width="500" /></td>
      <td><img src="https://github.com/user-attachments/assets/876d7690-e4aa-46e1-b643-b50170ba9737" width="500" /></td>
    </tr>
  </table>
    
  <br>
  
  > A terminal-based Python flashcard application built to assist in learning and practicing Chinese vocabulary through spaced repetition and command-line interactions.
  
  <div align="center">
    <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" alt="Python" />
    <img src="https://img.shields.io/badge/CLI_Tool-000000?style=for-the-badge&logo=gnometerminal&logoColor=white" alt="CLI" />
  </div>
  
  <br>
  
  **[`> ./go_to_source_code`](https://github.com/Michael-Steenkamp/Chinese)**
  
  <hr>
  </details>

  <details>
  <summary><code><b>drwxr-xr-x ./repo-space-shooter</b></code></summary>
  <br>
  
  <table>
    <tr>
    </tr>
  </table>
    
  <br>
  
  > A 2D space shooter game developed using the Unity Engine and C#. Features dynamic player movement, enemy spawning mechanics, and an integrated scoring system.
  
  <div align="center">
    <img src="https://img.shields.io/badge/Unity-100000?style=for-the-badge&logo=unity&logoColor=white" alt="Unity" />
    <img src="https://img.shields.io/badge/C%23-239120?style=for-the-badge&logo=c-sharp&logoColor=white" alt="C#" />
  </div>
  
  <br>
  
  **[`> ./go_to_source_code`](https://github.com/Michael-Steenkamp/SpaceShooter)**
  
  <hr>
  </details>
  
</div>
